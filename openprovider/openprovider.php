...
class Openprovider extends Module
{
    ...
    {
        $this->view = new View('tab_client_domain_contacts', 'default');
        $this->view->setDefaultView($this->default_module_view_path);
        ...
        foreach ($handles as $key => $handle) {
            ...
            $op_customers[$handle] = [
                ...
                'contact' => [
                    ...
                    'address' => $op_customer['address']['street'] . ' ' .
        ...
        if ($post) {
            ...
            $result = $this->createOrUpdateDomainContactsInOp($api, $op_domain['id'], $handles, $domain_contacts_from_op, $domain_contacts_from_post);
     ...
     */
    private function createOrUpdateDomainContactsInOp(
        OpenProviderApi $api,
        int $op_domain_id,
    ...
    ): string
    {
        ...
        foreach ($domain_handles as $type => $handle) {
            $create_or_update_or_nothing = 'create';
            if (
                ...
                $customers_from_op[$type]['last_name'] == $customers_to_update[$type]['last_name'] ||
                $customers_from_op[$type]['company_name'] == $customers_to_update[$type]['company_name']) &&
                array_count_values(array_values($domain_handles))[$handle] == 1
            ) {
                $create_or_update_or_nothing = 'update';
            }
            ...
            if ($create_or_update_or_nothing == 'update') {
                $update_customer_request = $api->call('modifyCustomerRequest', $args);

                $this->logRequest($api);

                if ($update_customer_request->getCode() != 0) {
                    return $update_customer_request->getMessage();
                }
